...
```{r}
  ...
  geom_label(position = position_jitter(height = 0, width = .3)) +
...
```{r}
unknown %>%
  ggplot(aes(x = trial_lang, y = unknown)) +
...
These are all judge to be quite typical examples of the object category denoted
...
item_sel %>%
  ungroup() %>%
  left_join(typic_full %>%
              select(item_id, trial_lang, pic_file) %>% unique()
            ) %>%
